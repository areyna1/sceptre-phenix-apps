    ...
    def pre_start(self):
        ...
        for node in nodes:
            ...
            with open(f'{self.startup_dir}/pst-start.sh', "w") as file_:
                utils.mako_serve_template("pst-start.mako", self.mako_templates_path, file_, server_endpoint=srv_endpoint, publish_endpoint=node.metadata.publish_endpoint, publish_rate=node.metadata.publish_rate)
