...
class Infrastructure:
    ...
    def factory(infrastructure: str):
        ...
        if infrastructure == 'wac':
            return WideAreaControl()
        return None
...
class WideAreaControl(Infrastructure):
    def __init__(self):
        super().__init__()
        super().register('wac')
        self.range = 0, 1000
    ...
    @staticmethod
    def create_device(device_type, device_name, protocol,reg_config,**kwargs):
        if type(device_type) == str and device_type.lower() == 'pmu':
            device_kwargs = {'range'        : (0, 1000),
                      'analog-read'         : kwargs.get('analog-read',
                            ['timestamp', 'voltage_mag', 'voltage_angle'])}
        elif type(device_type) == str and device_type.lower() == 'ras':
            device_kwargs = {'range'        : (0, 1000),
                      'analog-read-write'         : kwargs.get('analog-read-write',
                            ['timestamp', 'voltage_mag_avg', 'voltage_angle_avg'])}
        infra = infrastructure=WideAreaControl.INFRA
        return Device(device_type, device_name, protocol, reg_config,
                      infrastructure=infra, **device_kwargs)
