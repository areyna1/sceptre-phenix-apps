...
%SCEPTRE integration
%Initialize data structure to pass to mdoel hooks
%TODO eventually this will be replaced
%TODO: the PMU representation in PST is a WIP, this is just a notional standin to get the data structures working
%TODO: the RAS representation in PST is a WIP, this is just a notional standin to get the data structures working
data.pmus.timedelta=zeros(1,k);

data.pmus.timestamp = zeros(length(g.bus.bus_v(:, 1)), 1);
data.pmus.voltage_mag = zeros(length(g.bus.bus_v(:, 1)), 1);
data.pmus.voltage_angle = zeros(length(g.bus.bus_v(:, 1)), 1);
%TODO this assumes just one RAS. THis will need to be generalized. 
data.rass.timestamp_avg = zeros(1, 1);
data.rass.voltage_mag_avg = zeros(1, 1);
data.rass.voltage_angle_avg = zeros(1, 1);
modelHooks(0, data);
...
while (kt <= ktmax)
    ...
    for k = k_start:k_end
        ...
        % SCEPTRE integration
	%NOTE be careful of the index. I think we want j (which is set to k+1) here since that is the current values being assigned in this loop. 
 	%TODO the PMU representation in PST is a WIP, this is just a notional standin to get the data structures working.
	%TODO add time delay (use simple offset) to test RAS logic
        %Requirements on what can be fed to modelHooks.c: 
	data.pmus.timestamp = zeros(length(g.bus.bus_v(:, j)), 1) + t(k);	
	
        %ADDING TIME DELAY

        %data.pmus.timestamp(0,j) = data.pmus.timestamp(0,j)+50e-6;
        
        data.pmus.voltage_mag = real(g.bus.bus_v(:, j));	
	data.pmus.voltage_angle= real(g.bus.theta(:, j));	
 	
	disp('before modelHooks')
	data.rass
	data = modelHooks(1, data); %step
    	disp('after modelHooks')
	data.rass
        
        %z=sprintf("PST Timestamp: %d", t(k));
        %disp(z);

        %y=sprintf("RAS Timestamp Avg: %d", data.rass.timestamp_avg));
        %disp(y)

        data.pmus.timedelta(k) = t(k) - data.rass.timestamp_avg;
        x=sprintf("Timedelta: %d", data.pmus.timedelta(k));
        disp(x)
    end
...
while(flag == 0)
    ...
    % SCEPTRE intergration
    modelHooks(2, data); %term
end
