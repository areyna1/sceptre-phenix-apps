        ...
        for server in mapping.values():
            ...
            if self.metadata.get("use_rperf"):
                ...
                for client in server.clients.values():
                    ...
                    #server_cmd = f"{server.exe_path} --server --one-off --bind {server.ip} --json --port {client.port} --logfile {client.server_log_path} --idle-timeout {idle_timeout} --json-stream -t 300"
                    server_cmd = f"{server.exe_path} --server --one-off --bind {server.ip} --port {client.port} --logfile {client.server_log_path} --idle-timeout {idle_timeout} --json-stream -t 300"
                    ...
                    if node_info["os_types"][server.hostname] == "linux":
                        ...
                        server_cmd += f" --verbose --debug 2>1 >/iperf_server-log_client-{client.hostname}_server-{server.hostname}.log' --json-stream -t 300"
        ...
        for server in mapping.values():
            for client in server.clients.values():
                ...
                if self.metadata.get("use_rperf"):
                    ...
                    client_cmd = f"{client.exe_path} --client {server.ip} --port {client.port} --logfile {client.client_log_path} --bind {client.ip} --time {run_for} --cport {client.port+1000} --json-stream"
